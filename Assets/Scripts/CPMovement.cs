...
public class CPMovement : MonoBehaviour
{
    ...
    Vector3 duckScale = new Vector3 (1f, 0.5f, 1f);
	/*print() style */
    ...
    private Vector3 rotationDeltaStart;
    private Vector3 rotationDeltaEnd;
    ...
    private float distanceTravelledTemp = 0;
    ...
    private void Start()
	{
        ...
        if (playerLogic == null)
        {
            Debug.LogError("Player Logic is not assigned.");
        }
    }
	...
	private void Update()
	{
        ...
        rotationDeltaStart = transform.localRotation.eulerAngles;
        ...
        controllerInfo.isGrounded = _controller.isGrounded;
        controllerInfo.wasGrounded = wasGrounded;

        distanceTravelledTemp += temp;
        if (distanceTravelledTemp >= stepDistance && _controller.isGrounded)
        {
            distanceTravelledTemp = 0f;
            controllerInfo.didStep = true;
            inverter = !inverter;
        }
        else
        {
            controllerInfo.didStep = false;
        }
        if(_controller.isGrounded)
        controllerInfo.timeBetweenStepsNormalized = inverter ? distanceTravelledTemp / stepDistance : -(distanceTravelledTemp / stepDistance);
	...
	private void QueueJump()
	{
        ...
        {
            wishJump = true;
            controllerInfo.didPressJump = true;
        }
        if (Input.GetButtonUp("Jump"))
        {
            wishJump = false;
            controllerInfo.didPressJump = false;
        }
	}
	...
	private void OnGUI()
	{
        ...
        GUI.Label(new Rect(0, 45, 400, 100), "Is Grounded: " + _controller.isGrounded, style);
        //GUI.Label(new Rect(0, 60, 400, 100), "Is Grounded: " + _controller.isGrounded, style);
        //GUI.Label(new Rect(0, 45, 400, 100), "Ammo: " + WeaponModule.FireModule.AmmoModule.ammoInMag, style);
